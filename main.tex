...
\begin{itemize}
    \vspace{-1.75em}
    \item Editing \texttt{custom-rules.rules} and allow us to write custom Suricata rules using its \href{https://www.youtube.com/watch?v=d-t-8lJ2PII&t=829s}{\textbf{proper syntax}}. We will write two rules: one that detects any network http interactions specifically between the IP \codebox{27.43.100.29} and our \texttt{\$HOME\_NET}, and a second that detects any network interactions with any outside networks \texttt{\$EXTERNAL\_NET}. \\
...
alert http 27.43.100.29 any <-> $HOME_NET any (msg: "threat list alert";)
alert http $EXTERNAL_NET any <-> $HOME_NET any (msg: "external alert";)
\end{ubuntu}
\begin{itemize}
    \item After using \codebox{Ctrl+X} and \codebox{Y} followed by \codebox{ENTER} to save and exit, we can then \texttt{cd} back out again and run all three \texttt{.pcap} files through Suricata, which should alert us in accordance with our new custom rules. To do so, follow \href{https://asciinema.org/a/488029}{\textbf{this video}}. \\
    \textbf{Note:} Come back to this---having difficulties with configuration change (to ``cheat").
\end{itemize}
\end{orangebox}
