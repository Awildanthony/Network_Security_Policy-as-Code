...
\begin{itemize}
    ...
    Answer. 
    \item\textbf{How does a VPN's cryptography work? Why is it secure, and do you retain total anonymity?} \\
    Answer.
    \item\textbf{Like with overlay of security protocols, how can multiple services be using the same port, and why is it that you don't see port numbers registered above 1024-65535?} \\
    Answer.
    \item\textbf{Is there sense in security through obfuscation---say, running SSH through a non-default port? How can someone find out an SSH service is running on any port without prior knowledge of the port number?} \\
    Answer.
    \item\textbf{What are the IPSec, OpenVPN, and Wireguard protocols and how do they use encapsulation?} \\
    Answer. 
    \item\textbf{How does a VPN client (say, Ubuntu Linux) know to not leak data by sending traffic outside the VPN, thereby going through the normal network connection?} \\
    Answer.
